...
namespace Data_Logic_Layer
{
    public class DALAdminUser
    {
        ...
        public async Task<string> UpdateUser(UserDetail updatedUser)
        {
            try
            {
                var result = string.Empty;
                using (var transaction = await _context.Database.BeginTransactionAsync())
                {
                    try
                    {
                        ...
                        var userDetail = await _context.UserDetail.FirstOrDefaultAsync(x => x.UserId == updatedUser.Id);
                        if (userDetail != null)
                        {
                            userDetail.EmployeeId = updatedUser.EmployeeId;
                            userDetail.Department = updatedUser.Department;
                            userDetail.Status = updatedUser.Status;
                        }
                        else
                        {
                            // If UserDetail doesn't exist, create a new one
